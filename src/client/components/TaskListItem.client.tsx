...
function buildCompletionActions(t: Translate, task: TaskNode): MenuAction[] {
    ...
    return [{
        label: t('list.actions.markCompleted', 'Completed'),
...
function buildActions(t: Translate, task: TaskNode, canAct: boolean, currentUserKey: string): MenuAction[] {
    ...
    if (canAct && task.state === 'active') {
        actions.push({label: t('common.actions.start', 'Start'), mutation: UPDATE_TASK_STATE_MUTATION, variables: {id: task.id, state: 'started'}, patch: {state: 'started'}});
    }

    if (canAct && task.state === 'started') {
        actions.push({label: t('common.actions.suspend', 'Suspend'), mutation: SUSPEND_TASK_MUTATION, variables: {id: task.id}, patch: {state: 'suspended'}});
        actions.push(...buildCompletionActions(t, task));
    }

    if (canAct && task.state === 'suspended') {
        actions.push({label: t('common.actions.continue', 'Continue'), mutation: RESUME_TASK_MUTATION, variables: {id: task.id}, patch: {state: 'started'}});
    }
...
export default function TaskListItem({task: initialTask, currentUserKey, canReviewAll, graphqlEndpoint}: Readonly<TaskListItemProps>) {
    ...
    const actions = buildActions(t, task, canAct, currentUserKey);
    ...
    const menuItems = actions.length === 0
        ? [<MenuItem key="none" label={t('common.noActions', 'No actions available')} isDisabled/>]
    ...
    return (
        <div className="task-list-item__row">
            ...
            <p className="task-list-item__title">{task.title ?? t('common.untitledTask', 'Untitled task')}</p>
            {task.description && <p className="task-list-item__description">{task.description}</p>}
            {task.state === 'finished' && <p className="task-list-item__completed">{t('list.completed', 'Completed')}</p>}
            <div ref={anchorRef}>
                <Button
                    ...
                    aria-label={isMenuOpen
                        ? t('list.actions.hideMenu', 'Hide task actions')
                        : t('list.actions.showMenu', 'Show task actions')}
